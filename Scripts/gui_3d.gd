...
func _input(event: InputEvent) -> void:
	if event is InputEventMouse:
		...
		if collider == node_area and collider != old_collider:
			_mouse_entered_area()
		if collider != node_area and collider != old_collider:
			_mouse_exited_area()
		...
		if node_raycast.get_collider() != node_area:
			return
		
		
		var event_pos3D = node_raycast.get_collision_point()
		
		#var event_pos3D = space_state.intersect_ray(query).position
...
func rotate_area_to_billboard() -> void:
	...
	if billboard_mode > 0:
		...
		node_area.global_rotation = camera.global_rotation
		# Y-Billboard: Lock Y rotation, but gives bad results if the camera is tilted.
		if billboard_mode == 2:
			look = Vector3(look.x, 0, look.z)

		#node_area.look_at(look, Vector3.LEFT)
